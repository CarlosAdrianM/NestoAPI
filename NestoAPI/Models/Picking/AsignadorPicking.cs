...
namespace NestoAPI.Models.Picking
{
    public class AsignadorPicking
    {
        ...
        public void Ejecutar()
        {
            if (pedidos.Count == 0)
            {
                return; // no hay ningún pedido para sacar picking
            }
            ...
            foreach(PedidoPicking pedido in pedidos)
            {
                foreach (LineaPedidoPicking linea in pedido.Lineas)
                {
                    ...
                    } else
                    {
                        ...
                        lineaActual = db.LinPedidoVtas.Local.FirstOrDefault(l => l.Empresa == pedido.Empresa && l.Número == pedido.Id && l.TipoLinea == linea.TipoLinea && l.Producto == linea.Producto);
                    }
