    ...
    public class RellenadorPickingService : IRellenadorPickingService
    {
        ...
        private List<PedidoPicking> Ejecutar()
        {
            ...
            return pedidos.Select(p => new PedidoPicking
            {
                ...
                // NestoAPI#542: el modo de facturación (3 = lo que falte va a Recoger, no a pendiente)
                MantenerJunto = p.MantenerJunto,
                ModoFacturacion = p.ModoFacturacion,
                ...
                EsPrecioPublicoFinal = (p.LinPedidoVtas.FirstOrDefault(l => Constantes.FormasVenta.PRECIO_PUBLICO_FINAL.Contains(l.Forma_Venta)) != null) && p.Cliente.Estado == Constantes.Clientes.Estados.PUBLICO_FINAL && p.Vendedor != null &&  p.Vendedor.Trim() == "NV",
