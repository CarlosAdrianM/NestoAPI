...
--
-- IMPORTANTE: esta tabla vive en la base de datos de Identity (BD `AspNetIdentity`,
-- connection string "NVIdentity", servidor DC2016, Integrated Security=SSPI),
-- NO en la base de datos principal de negocio ("NestoConnection").
...
-- usan esta tabla.
--
-- Permisos: NVIdentity NO usa GRANTs explícitos. La app accede con las cuentas
-- máquina del IIS (NUEVAVISION\RDS2016$ y NUEVAVISION\RDS2019$), que son
-- miembros de db_datareader + db_datawriter. La nueva tabla hereda el acceso
-- automáticamente. NO añadir GRANTs aquí (a diferencia de los scripts contra
-- NestoConnection, que sí los necesitan para [ABORRAR\sqlServerSvc]).
