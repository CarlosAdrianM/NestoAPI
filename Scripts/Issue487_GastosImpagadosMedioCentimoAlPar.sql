/*
    Issue487 - prdContabilizarImpagadosSepa: la comision en MEDIO CENTIMO EXACTO se redondea al par.
    Caso real (01/09/26): recibo NV2614133/1 de 100,50 -> comision 1,005. ROUND de SQL Server sube
    (1,01 + 0,21 = 1,22); Caixabank resuelve el medio centimo al par (1,00 + 0,21 = 1,21).
    CAMBIA UNA SOLA COSA (buscar "Issue487" en el cuerpo): en el redondeo de #441, si la milesima es 5
    exacta y el centimo anterior es par, se trunca (al par hacia abajo); si es impar, ROUND ya sube al
    par. Mismo tratamiento en el paso del IVA. El resto de recibos no cambia:
      100,50 -> 1,00 + 0,21 = 1,21  (antes 1,22)   <- el caso de #487
      159,26 -> 1,59 + 0,33 = 1,92  (igual que #441)
      145,73 -> 1,46 + 0,31 = 1,77  (igual que #441)
      101,50 -> 1,02 + 0,21 = 1,23  (al par: 1,015 sube; pendiente de confirmar con un recibo asi)
    El cuerpo es el volcado EXACTO de produccion (sys.sql_modules, 17/09/2026, identico al script
    de #441 salvo CREATE/ALTER) con ese unico parche. Ejecutar en SSMS como sa contra NV.
    Verificacion en vivo: el proximo fichero de impagados con un recibo acabado en ,50 (o ,5X con
    comision 1 %) debe cuadrar centimo a centimo con el extracto del banco.
*/
...
declare @milesimasGastos as bigint -- Issue487
declare @ivaGastos as money -- Issue487
...
while @i <= @cuentaTotal begin
	...
	*/
	...
	-- Issue441 (01/09/26): Caixabank liquida en DOS pasos, redondeando cada uno: la comision
	-- a centimos ANTES del IVA, y el IVA sobre esa base ya redondeada. El round unico de antes
	-- (base * 1,21) descuadraba +-1 centimo por recibo contra el banco (asientos 1199417/1199418:
	-- nosotros 1,93/1,76, el banco 1,92/1,77 -- un caso en cada direccion, mismas reglas, otro orden).
	-- Issue487 (17/09/26): el medio centimo EXACTO (100,50 x 1 % = 1,005) el banco lo resuelve AL PAR
	-- (1,00); ROUND lo sube (1,01). Solo cambia ese caso: milesima 5 exacta y centimo anterior par ->
	-- se trunca; con centimo anterior impar ROUND ya sube al par. Mismo tratamiento en el paso del IVA.
	set @milesimasGastos = cast(round(@gastosImpagado * 1000, 0) as bigint)
	if abs(@gastosImpagado * 1000 - @milesimasGastos) < 0.00001 and @milesimasGastos % 10 = 5 and (@milesimasGastos / 10) % 2 = 0
		set @gastosImpagado = round(@gastosImpagado, 2, 1)
	else
		set @gastosImpagado = round(@gastosImpagado, 2)
	set @ivaGastos = @gastosImpagado * @porcIVA
	set @milesimasGastos = cast(round(@ivaGastos * 1000, 0) as bigint)
	if abs(@ivaGastos * 1000 - @milesimasGastos) < 0.00001 and @milesimasGastos % 10 = 5 and (@milesimasGastos / 10) % 2 = 0
		set @ivaGastos = round(@ivaGastos, 2, 1)
	else
		set @ivaGastos = round(@ivaGastos, 2)
	set @gastosImpagado = @gastosImpagado + @ivaGastos
