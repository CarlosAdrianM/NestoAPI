-- NestoAPI#480: prdCrearRemesaIso20022 emite TODO como RCUR y deja de comprobar las secuencias
-- de los contactos. Ejecutar como sa en SSMS ANTES de publicar la NestoAPI que contabiliza
-- agrupando solo por fecha (si no, el fichero podria seguir sacando un bloque FRST que la
-- contabilidad ya no separa). El UPDATE final que pasa los FRST a RCUR se conserva: la columna
-- sigue siendo informativa en la ficha de Nesto.
-- Generado a partir de la definicion en produccion del 10/09/2026 (sys.sql_modules) con solo
-- estos dos cambios: (1) fuera el bloque de comprobacion/raiserror de secuencias, (2) 'RCUR'
-- fijo en el insert de #remesa.
...
-- NestoAPI#480 (10/09/26): desde el Rulebook SEPA Core de 2016 la secuencia FRST/RCUR es
-- informativa y el banco acepta RCUR en el primer cobro (la remesa 10930 lo demostro). Todo va
-- como RCUR: ya no se lee CCC.Secuencia ni se comprueba que los contactos coincidan (la
-- comprobacion bloqueaba TODAS las remesas por un cliente incoherente, NestoAPI#368), y generar el
-- fichero dos veces da el mismo resultado (antes el volteo del final cambiaba los bloques y el
-- banco abonaba 2 bloques contra 3 asientos).
...
insert into #remesa
	select e.[nº orden], e.Numero, e.contacto, -e.Importe, e.[CIF/NIF],
            ...
            e.CCC, e.[Nº Documento], e.Efecto, e.FechaVto, c.FechaMandato, 'RCUR', -- NestoAPI#480: siempre RCUR
...
---------------------------------------------------------------------
	...
	select @nombreCliente = replace(@nombreCliente, '&', '&amp');
