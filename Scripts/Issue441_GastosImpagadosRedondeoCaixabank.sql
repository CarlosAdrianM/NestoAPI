/*
    Issue441 - prdContabilizarImpagadosSepa: los gastos descuadraban +-1 centimo con Caixabank.

    CAMBIA UNA SOLA COSA (buscar "Issue441" en el cuerpo): el redondeo del gasto pasa de
    round(base * (1+IVA), 2) a dos pasos como liquida el banco: round(base, 2) y despues
    + round(base_redondeada * IVA, 2).

    Verificacion con los dos casos reales del 28/08/2026 (comision 1%, IVA 21%):
      159,26 -> 1,59 + 0,33 = 1,92  (antes 1,93)
      145,73 -> 1,46 + 0,31 = 1,77  (antes 1,76)
    El caso del tope (max 2,48) da 3,00 por ambas vias: no cambia.

    El cuerpo es el volcado EXACTO de produccion (sys.sql_modules, 01/09/2026, 19.707 chars,
    completo y verificado hasta su END final -- el primer intento de volcado cortaba a 8K y
    se noto porque faltaba el END) con ese unico parche. Ejecutar en SSMS como sa contra NV.
    Verificacion en vivo: el proximo fichero de impagados debe cuadrar centimo a centimo
...
while @i <= @cuentaTotal begin
	...
	*/
	...
	set @gastosImpagado = @gastosImpagado + round(@gastosImpagado * @porcIVA, 2)
